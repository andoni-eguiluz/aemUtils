...
import net.eguiluz.aem.utils.gui.DPFTipoArbolAnt;
import net.eguiluz.aem.utils.PopupMenu;

/** Ventana principal de AEsync para funcionamiento interactivo
 * @author andoni
 */
public class DirProcessFrameAnt extends JFrame implements TreeSelectionListener, MouseListener {
    ...
    private JPanel[] pArbol = new JPanel[DPFTipoArbolAnt.values().length];

        private void crearPanelDeJTree( DPFTipoArbolAnt tipoArbol ) {
            pArbol[tipoArbol.ordinal()] = new JPanel( new BorderLayout() );
            pArbol[tipoArbol.ordinal()].setBorder( BorderFactory.createTitledBorder( DPFTipoArbolAnt.nombreTipo[tipoArbol.ordinal()]));
            pArbol[tipoArbol.ordinal()].add( new JScrollPane( arbol[tipoArbol.ordinal()] ) );
        }
        private void muestraNumeroFicherosEnPanel( DPFTipoArbolAnt tipoArbol ) {
            TitledBorder tb = (TitledBorder) pArbol[tipoArbol.ordinal()].getBorder();
            tb.setTitle( DPFTipoArbolAnt.nombreTipo[tipoArbol.ordinal()] + " (" + numNodos[tipoArbol.ordinal()] + ")" );
        ...
        private void verPanelArbolEnPantalla( DPFTipoArbolAnt tipoArbol, int numPantalla) {
        ...
        private void recalcVentanasNoEnPantalla() {
            boolean ventanaEnPantalla[] = new boolean[ DPFTipoArbolAnt.values().length ];
            for (int i = 0; i<6; i++) {
                Component c = null;
                switch (i) {
            ...
            for (int i = 0; i<ventanaEnPantalla.length; i++) {
                if (!ventanaEnPantalla[i]) {
                    if (!soloConteoIgnorados || (DPFTipoArbolAnt.values()[i]!=DPFTipoArbolAnt.DPF_IGNORADOS_EN_FUENTE && DPFTipoArbolAnt.values()[i]!=DPFTipoArbolAnt.DPF_IGNORADOS_EN_DESTINO))
                        cbArbolesNoVistos.addItem( DPFTipoArbolAnt.nombreTipo[i] );
                }
            }
        }
     ...
     */
    public DirProcessFrameAnt( File dirFuente, File dirDestino, boolean soloConteoIgnorados ) {
...
//		SwingUtilities.updateComponentTreeUI( this );
        ...
        for (DPFTipoArbolAnt ta : DPFTipoArbolAnt.values()) {
            if (DPFTipoArbolAnt.sonDeFuente[ta.ordinal()])
        ...
        pCentro = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true);
            ...
            pCentro1 = new JSplitPane(JSplitPane.VERTICAL_SPLIT, true);
                ...
                verPanelArbolEnPantalla(DPFTipoArbolAnt.DPF_SOLO_EN_FUENTE, 1);
                verPanelArbolEnPantalla(DPFTipoArbolAnt.DPF_SOLO_EN_DESTINO, 4);
...
//                pCentro1.setBottomComponent( pArbol[DPFTipoArbol.DPF_SOLO_EN_DESTINO.ordinal()] );
            pCentro23 = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true);
                ...
                pCentro2 = new JSplitPane(JSplitPane.VERTICAL_SPLIT, true);
                    ...
                    verPanelArbolEnPantalla(DPFTipoArbolAnt.DPF_MODERNO_EN_FUENTE, 2);
                    verPanelArbolEnPantalla(DPFTipoArbolAnt.DPF_MODERNO_EN_DESTINO, 5);
...
//                    pCentro2.setBottomComponent( pArbol[DPFTipoArbol.DPF_MODERNO_EN_DESTINO.ordinal()] );
                pCentro3 = new JSplitPane(JSplitPane.VERTICAL_SPLIT, true);
                    ...
                    verPanelArbolEnPantalla(DPFTipoArbolAnt.DPF_IGUALES, 3);
                    verPanelArbolEnPantalla(DPFTipoArbolAnt.DPF_ERRORES_COPIA, 6);
...
//                    pCentro3.setBottomComponent( pArbol[DPFTipoArbol.DPF_IGNORADOS_EN_DESTINO.ordinal()] );
        ...
        cbVerOtroArbol.addActionListener( new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (cbVerOtroArbol.getSelectedIndex() >= 0 && cbArbolesNoVistos.getSelectedIndex() >= 0) {
                    ...
                    int ventanaNoEnPantalla = DPFTipoArbolAnt.valueOfDescription( cbArbolesNoVistos.getSelectedItem().toString() );
                    if (ventanaNoEnPantalla >= 0) {
                        verPanelArbolEnPantalla( DPFTipoArbolAnt.values()[ventanaNoEnPantalla], cbVerOtroArbol.getSelectedIndex()+1 );
                    } else {  // Vacío
                    ...
                    synchronized (lastClickedPathCoords) {
                        if (lastClickedPathCoords.getX() != -10000) {  // Si no ha habido doble click, sacar el popup
                            if (myMouseEvent.getSource() instanceof Component) {
                                DPFTipoArbolAnt tipoA = getTipoArbol( (Component)myMouseEvent.getSource() );
                                if (tipoA != null) {
    ...
    private int[] numNodos = new int[ DPFTipoArbolAnt.values().length ];
    private boolean[] mostrarExpandido = { false, false, true, true, true, true, false, true, true };
    private JTree[] arbol = new JTree[ DPFTipoArbolAnt.values().length ];
    private DefaultMutableTreeNode[] raiz = new DefaultMutableTreeNode[ DPFTipoArbolAnt.values().length ];
    private DefaultMutableTreeNode[] ultimoDir = new DefaultMutableTreeNode[ DPFTipoArbolAnt.values().length ];
    private TreePath[] ultimoDirPath = new TreePath[ DPFTipoArbolAnt.values().length ];
    private String[] ultimoDirString = new String[ DPFTipoArbolAnt.values().length ];


    // Método privado de crear los árboles
    private JTree crearArbol( String nomRaiz, DPFTipoArbolAnt tipoArbol ) {
    ...
    private void irARaiz( DPFTipoArbolAnt tipoArbol ) {
        ultimoDir[tipoArbol.ordinal()] = raiz[tipoArbol.ordinal()];
        ultimoDirPath[tipoArbol.ordinal()] = new TreePath( ultimoDir[tipoArbol.ordinal()] );
    ...
    private void subirUnNivelUltimoAccedido( DPFTipoArbolAnt tipoArbol ) {
    ...
    private boolean existeFicheroEnUltimoAccedido( String fichero, DPFTipoArbolAnt tipoArbol ) {
    ...
    private void crearOBuscarDirEnUltimoAccedido( String nuevoPathDirRelativo, DPFTipoArbolAnt tipoArbol ) {
        do {
    ...
    private void crearOBuscarDir( String nuevoPathDir, DPFTipoArbolAnt tipoArbol ) {
    ...
    public void nuevoFichero( String nomFic, DPFTipoArbolAnt tipoArbol ) {
        // getAccess( "nuevoFic " + tipoArbol.toString() );
        numNodos[tipoArbol.ordinal()]++;
        if (soloConteoIgnorados && (tipoArbol==DPFTipoArbolAnt.DPF_IGNORADOS_EN_DESTINO || tipoArbol==DPFTipoArbolAnt.DPF_IGNORADOS_EN_FUENTE)) {
            if (tipoArbol==DPFTipoArbolAnt.DPF_IGNORADOS_EN_FUENTE)
