...
import net.eguiluz.aem.utils.BackupFiles;
import net.eguiluz.aem.utils.Contador;
import net.eguiluz.aem.utils.VentanaEdicionGenericaConfig;
import net.eguiluz.aem.utils.tabla.iu.Threadable;
import net.eguiluz.aem.utils.tabla.iu.VentanaDatos;
import net.eguiluz.aem.utils.tabla.iu.VentanaDatos.EventoEnCelda;
import net.eguiluz.aem.utils.tabla.iu.VentanaGeneral;
...
public class EdicionRapidaTabla {
	...
	private static VentanaEdicionGenericaConfig vConfig;
	
	public static void main(String[] args) {
		vConfig = new VentanaEdicionGenericaConfig( FIC_CONFIGURACION, Config.class, CONFIG_MENSAJES, CONFIG_DEFECTO, CONFIG_TIPOS );
		boolean ok = vConfig.mostrar();
		if (!ok) return;
		pathBD = vConfig.getProp( Config.PATH_BD );
		nombreTabla = vConfig.getProp( Config.NOMBRE_TABLA );
		BackupFiles.makeBakFiles( pathBD, 2, false );  // Crea backup
		...
		if (colNames!=null && colTypes!=null) {
			...
			vg.addMenuAccion( "Cambiar config", (e) -> vConfig.mostrar() );
			vg.addMenuAccion( "Guardar config", (e) -> { ventanaTabla.guardaProps(); vConfig.saveProps(); } );
			...
			ventanaTabla.setVentanaConfig( vConfig );
			ventanaTabla.cargaProps();
