package net.eguiluz.aem.utils.tabla.iu;
...
public class VentanaDatos extends JInternalFrame {
	...
	/** Define la ventana de configuración para esta ventana
	 * @param vConfig	Ventana de configuración a utilizar
	 */
	public void setVentanaConfig( VentanaEdicionGenericaConfig vConfig ) {
		this.vConfig = vConfig;
	}
		private transient Runnable rCargaProps;

	/** Carga las propiedades de un fichero de configuración (si está definido - método {@link setVentanaConfig(VentanaEdicionGenericaConfig)}
	 * (posición, ancho, alto, anchura de columnas)
	 */
	public void cargaProps() {
		if (rCargaProps==null) rCargaProps = new Runnable() {
			@Override
			public void run() {
				if (vConfig!=null) {
					int x = vConfig.getPropInt( "vent-" + tipo + "-x", getX() );
					int y = vConfig.getPropInt( "vent-" + tipo + "-y", getY() );
					int ancho = vConfig.getPropInt( "vent-" + tipo + "-width", getWidth() );
					int alto = vConfig.getPropInt( "vent-" + tipo + "-height", getHeight() );
					setBounds( x, y, ancho, alto );
					for (int col=0; col<tDatos.getModel().getColumnCount(); col++) {
						int anchoCol = vConfig.getPropInt( "vent-" + tipo + "-col" + col );
						if (anchoCol!=-1) {
							tDatos.getColumnModel().getColumn(col).setPreferredWidth( anchoCol );
						}
					}
					tDatos.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);
				}
			}
		};
		if (SwingUtilities.isEventDispatchThread()) rCargaProps.run(); else SwingUtilities.invokeLater( rCargaProps );
	}

	/** Guarda las propiedades de la ventana en fichero de configuración (si definido)
	 * (posición, anchura, altura, anchura de columnas)
	 */
	public void guardaProps() {
		if (vConfig!=null) {
			vConfig.setProp( "vent-" + tipo + "-x", "" + getX() );
			vConfig.setProp( "vent-" + tipo + "-y", "" + getY() );
			vConfig.setProp( "vent-" + tipo + "-width", "" + getWidth() );
			vConfig.setProp( "vent-" + tipo + "-height", "" + getHeight() );
			for (int col=0; col<tDatos.getColumnModel().getColumnCount(); col++) {
				int anchoCol = tDatos.getColumnModel().getColumn(col).getWidth();
				vConfig.setProp( "vent-" + tipo + "-col" + col, "" + anchoCol );
			}
		}
	}
